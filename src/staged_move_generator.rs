...
use crate::{
    ...
    board::{
        Board, CASTLE_BLACK_KING_FLAG, CASTLE_BLACK_KING_ROOK_SQ_IDX, CASTLE_BLACK_KING_SQ_IDX,
        CASTLE_BLACK_QUEEN_FLAG, CASTLE_BLACK_QUEEN_ROOK_SQ_IDX, CASTLE_WHITE_KING_FLAG, CASTLE_WHITE_KING_ROOK_SQ_IDX,
        CASTLE_WHITE_KING_SQ_IDX, CASTLE_WHITE_QUEEN_FLAG, CASTLE_WHITE_QUEEN_ROOK_SQ_IDX, PIECE_BISHOP, PIECE_KING,
        PIECE_KNIGHT, PIECE_MASK, PIECE_PAWN, PIECE_QUEEN, PIECE_ROOK, Squares, file_8x8, rank_8x8,
    },
...
impl StagedMoveGenerator {
    ...
    fn generate_pending_moves(
        ...
        if !self.prune_quiets_non_promos {
            ...
            if board.castling_rights != 0 {
                Self::add_castling_moves(board, &mut self.move_buf);
            }
        }
    ...
    pub fn add_castling_moves(board: &Board, moves: &mut ArrayVec<ScoredMove, MOVE_ARRAY_SIZE>) {
        if CHESS960.load(std::sync::atomic::Ordering::Relaxed) {
            Self::add_castling_moves_chess960(board, moves);
        } else {
    ...
    #[inline(never)]
    pub fn add_castling_moves_chess960(board: &Board, moves: &mut ArrayVec<ScoredMove, MOVE_ARRAY_SIZE>) {
        if board.white_to_move {
            if board.castling_rights & CASTLE_WHITE_QUEEN_FLAG != 0 {
